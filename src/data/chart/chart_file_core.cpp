...
// ============================================================================
// Core Operations - Global State
// ============================================================================
static MappedFile gFile;
static int64_t gLength = 0;

// Core functions to be used by both APIs
static bool core_loadChart(const char* path) {
    if (!gFile.open(path))
        return false;
    gLength = gFile.size();
    return true;
...
static void core_destroyChart() {
    gFile.close();
    gLength = 0;
}

static int64_t core_getNote(int64_t index) {
    return gFile.get(index);
}

static void core_setNote(int64_t index, int64_t value) {
    gFile.set(index, value);
}

static int64_t core_getLength() {
    return gLength;
}