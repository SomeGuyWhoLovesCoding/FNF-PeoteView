...
// Project
import rhythm.NoteskinHandle.BasicNoteskinClip;
import rhythm.NoteskinHandle.NoteskinConfig;
import rhythm.NoteskinHandle.NoteskinData;
import rhythm.NoteskinHandle.NoteskinReceptorProperties;
import rhythm.NoteskinHandle.TextureRotation;
import rhythm.NoteskinRuntimeHelper;
...
class NoteskinGUISprite implements Element {
	...
	inline function changeID(id:Int) {
		if (id < 0)
			id = 0;
		if (id >= ATLAS_FRAMES.length)
			id = ATLAS_FRAMES.length - 1;
...
private class NoteskinEditorClipEditor {
	...
	function updateClipInConfig(index:Int, editState:EditState, clip:BasicNoteskinClip) {
		...
		if (clips != null && index < clips.length) {
			...
			switch (editState) {
				case IDLE:
					currentClip.idle = clip;
				case COLOR:
					currentClip.color = clip;
				case PRESS:
					currentClip.press = clip;
				case CONFIRM:
					currentClip.confirm = clip;
				case HOLD_BODY:
					currentClip.holdBody = clip;
				case HOLD_TAIL:
					currentClip.holdTail = clip;
				default:
			}
		}
	}
	...
	function getClipValue(clip:BasicNoteskinClip):Int {
		switch (state.selectedProperty) {
			...
			case "rotation":
				return clip.rotation.toDegrees();
			default:
				return 0;
		}
	}
	...
	function adjustSelectedValue(amount:Int) {
		...
		if (!isReceptorOnScreen(state.selectedIndex))
			return;
		...
		switch (state.selectedProperty) {
			case "clipX":
				basicClip.clipX -= amount;
			case "clipY":
				basicClip.clipY -= amount;
			case "clipW":
				basicClip.clipW += amount;
			case "clipH":
				basicClip.clipH += amount;
			case "offsX":
				basicClip.offsX += amount;
			case "offsY":
				basicClip.offsY += amount;
			case "clipIndex":
				...
				while (state.noteskinData.clip.length <= idxArr[state.selectedIndex] && state.noteskinData.clip.length < MAX_KEYS) {
			...
			case "rotation":
				if (amount > 0)
					basicClip.rotation = basicClip.rotation.next();
				else if (amount < 0)
					basicClip.rotation = basicClip.rotation.prev();
			default:
	...
	function toggleEditMode() {
		...
		switch (state.editMode) {
			case CLIP_POS:
				if (state.selectedProperty == "clipW" || state.selectedProperty == "clipH" || state.selectedProperty == "offsX"
					|| state.selectedProperty == "offsY" || state.selectedProperty == "clipIndex")
					state.selectedProperty = "clipX";
			case CLIP_SIZE:
				if (state.selectedProperty == "clipX" || state.selectedProperty == "clipY" || state.selectedProperty == "offsX"
					|| state.selectedProperty == "offsY" || state.selectedProperty == "clipIndex")
					state.selectedProperty = "clipW";
			case OFFSET:
				if (state.selectedProperty == "clipX" || state.selectedProperty == "clipY" || state.selectedProperty == "clipW"
					|| state.selectedProperty == "clipH" || state.selectedProperty == "clipIndex")
					state.selectedProperty = "offsX";
			case CLIP_ID:
				if (state.selectedProperty == "clipX" || state.selectedProperty == "clipY" || state.selectedProperty == "clipW"
					|| state.selectedProperty == "clipH" || state.selectedProperty == "offsX" || state.selectedProperty == "offsY")
					state.selectedProperty = "clipIndex";
	...
	function getSelectedNote():Note {
		if (state.strumline == null || state.selectedIndex >= state.strumline.length)
			return null;
	...
	function rotateCurrentClip(direction:Int) {
		...
		var newRotation:TextureRotation = if (direction > 0) clip.holdBody.rotation.next() else clip.holdBody.rotation.prev();
	...
	function rotateSustainSprite(receptorIndex:Int, degrees:Float) {
		if (state.spriteSheetMode)
			return;
		if (receptorIndex < 0 || receptorIndex >= state.maxReceptors)
			return;
	...
	**/
	...
	function isReceptorOnScreen(index:Int):Bool {
		if (state.strumline == null || index < 0 || index >= state.strumline.length)
			return false;
		...
		var sw = note.w * scale;
		var sh = note.h * scale;
		return (note.x + sw > 0 && note.x < Main.INITIAL_WIDTH && note.y + sh > 0 && note.y < Main.INITIAL_HEIGHT);
	}
...
private class NoteskinEditorManiaManager {
	...
	static function makeManiaConfig(keyCount:Int, gap:Int = 112, scale:Float = 1.0):NoteskinConfig {
		...
		return {
			...
			gap: gap,
			scale: scale,
			idleIndexes: idx.copy(),
			pressIndexes: idx.copy(),
			colorIndexes: idx.copy(),
			confirmIndexes: idx.copy(),
			holdBodyIndexes: idx.copy(),
			holdTailIndexes: idx.copy()
		};
	}
	...
	function finalizeImport(foundConfig:NoteskinConfig, keyCount:Int, entries:Array<{
		group:String,
		clipType:String,
		x:Int,
		y:Int,
	...
	static function applyClipTypeData(clip:NoteskinReceptorProperties, clipType:String, clipData:BasicNoteskinClip) {
		switch (clipType) {
			...
			case "press":
				clip.press = clipData;
			case "confirm":
				clip.confirm = clipData;
			case "color":
				clip.color = clipData;
			case "holdBody":
				clip.holdBody = clipData;
			case "holdTail":
				clip.holdTail = clipData;
			default:
		}
	}
	...
	static function parseSubTextureElement(elem:Xml):{
		x:Int,
		y:Int,
		w:Int,
		h:Int,
		...
		return {
			...
			w: sw,
			h: sh,
			offsX: sox != null ? sox : 0,
			offsY: soy != null ? soy : 0
		};
	}
			...
			if (managerHandle != null) {
				...
				if (managerHandle.loaded && !wasLoaded) {
					...
					if (NoteskinEditor.noteProg != null)
						managerHandle.setProgramsTexture(NoteskinEditor.noteProg);
					if (NoteskinEditor.sustainProg != null)
						managerHandle.setProgramsTexture(NoteskinEditor.sustainProg);
	...
	function switchMania(direction:Int) {
		...
		if (newIndex < 0)
			newIndex = totalManias - 1;
		if (newIndex >= totalManias)
			newIndex = 0;
	...
	function fillMissingManias(maxKeys:Int) {
		...
		for (i in 1...maxKeys) {
			if (existingKeyCounts.indexOf(i) != -1)
				continue;
	...
	function confirmConfirmationPopup() {
		...
		switch (state.confirmationPopupType) {
			...
			case IMPORT_VANILLA:
				importFromAtlas();
			case IMPORT_LETTERED:
				importFromAtlas18K();
			case SWITCH_NOTESKIN:
				switchNoteskin();
			case EXIT:
	...
	function adjustGap(amount:Int) {
		...
		if (state.currentConfig.gap < 1)
			state.currentConfig.gap = 1;
		if (state.currentConfig.gap > 500)
			state.currentConfig.gap = 500;
	...
	function importFromAtlas() {
		...
		try {
			...
			for (elem in root.elements()) {
				...
				if (clipType == null) {
					...
					for (c in knownColors) {
						if (trimmed.indexOf(c) != -1) {
							foundColor = c;
							break;
						}
					}
					if (foundColor == null && trimmed.indexOf("pruple") != -1)
						foundColor = "pruple";
			...
			if (dirGroupsFound.length > 0) {
				...
				if (colorGroupsFound.length > 0)
					trace('Color groups: ${colorGroupsFound.join(", ")}');
			} else {
	...
	function importFromAtlas18K() {
		...
		try {
			...
			for (elem in root.elements()) {
				...
				if (sp == -1 && trimmed.length > 1)
					continue; // no space, >1 char = skip
				if (sp != -1 && sp != 1)
					continue; // first word >1 char = skip
...
private class NoteskinEditorRenderer {
	...
	static function initStaticBuffers() {
		try {
			...
			if (NoteskinEditor.backgroundProg == null) {
				...
				GridBackgroundSprite.init(NoteskinEditor.backgroundProg, NoteskinEditor.BACKGROUND_TEXTURE_NAME, gridTex);
			}
	...
	static function initStaticGUISprites() {
		if (NoteskinEditor.guiTextureLoaded)
			return;
	...
	static function initStaticGridSprites() {
		if (NoteskinEditor.backgroundSprites.length > 0)
			return;
		if (NoteskinEditor.backgroundBuf == null)
			return;
		...
		if (Main.INITIAL_HEIGHT > S)
			S = Main.INITIAL_HEIGHT;
		S = Math.ceil(S);
	...
	static function initStaticTexts(display:Display) {
		...
		if (NoteskinEditor.saveButtonText == null) {
			NoteskinEditor.saveButtonText = new Text("SAVE_NOTESKIN_BTN", 0, 0, display, "SAVE NOTESKIN", "vcr");
		...
		if (NoteskinEditor.importButtonText == null) {
			NoteskinEditor.importButtonText = new Text("IMPORT_ATLAS_BTN", 0, 0, display, "IMPORT VANILLA XML", "vcr");
		...
		if (NoteskinEditor.importButton18KText == null) {
			NoteskinEditor.importButton18KText = new Text("IMPORT_ATLAS_18K_BTN", 0, 0, display, "IMPORT LETTERED XML", "vcr");
		...
		if (NoteskinEditor.switchNoteskinButtonText == null) {
			NoteskinEditor.switchNoteskinButtonText = new Text("SWITCH_NOTESKIN_BTN", 0, 0, display, "SWITCH NOTESKIN", "vcr");
	...
	static function initStaticReceptors(state:NoteskinEditor) {
		...
		state.strumline = new Strumline(Std.int(pos.x), Std.int(pos.y), state.noteskinHandle, gap, scale, state.maxReceptors, null);
	...
	function clampPreviewScroll() {
		...
		if (state.previewScrollOffset > maxScroll)
			state.previewScrollOffset = maxScroll;
		if (state.previewScrollOffset < minScroll)
			state.previewScrollOffset = minScroll;
	}
	...
	function createSustains() {
		...
		for (i in 0...state.maxReceptors) {
			...
			var sustain = new Sustain(Std.int(pos.x), Std.int(pos.y), 100, 30, state.noteskinHandle, -90, 1.0, 1.0, 0);
...
private class NoteskinEditorUI {
	...
	static function getBgColor(action:String):Int {
		if (action.startsWith("mode_"))
			return COL_MODE_BG;
		if (action.startsWith("sust_"))
			return COL_MODE_BG;
		if (action == "mania_create" || action == "show_instructions")
			return COL_DANGER_BG;
	...
	function buildGUIPanel() {
		...
		if (readoutX < leftPanelX + leftPanelW + 8)
			readoutX = leftPanelX + leftPanelW + 8;
	...
	function updateButtonHighlights() {
		for (btn in state.guiButtons) {
			...
			switch (btn.action) {
				case "mode_clippos":
					isActive = (state.editMode == CLIP_POS);
				case "mode_clipsize":
					isActive = (state.editMode == CLIP_SIZE);
				case "mode_offset":
					isActive = (state.editMode == OFFSET);
				case "mode_clipid":
					isActive = (state.editMode == CLIP_ID);
				case "mode_global":
					isActive = (state.editMode == GLOBAL_TRANSFORM);
				case "sustain_toggle":
					isActive = state.showSustainPreview;

				case "show_instructions":
					isActive = state.showInstructionsPopup;
				default:
			}
	...
	function buildInstructionsPopupText():String {
		return "#M1#KEYBOARD / MOUSE INSTRUCTIONS#M1#\n\n"
	...
	function renderCreateManiaPopup() {
		...
		var popupText = "#M6#=== CREATE NEW MANIA ===#M6#\n"
...
private class NoteskinEditorInputHandler {
		...
		switch (key) {
			...
			case KeyCode.DOWN:
				if (state.isShiftPressed) {
					...
					if (state.globalScaleMode) {
						state.currentConfig.scale -= 0.05;
						if (state.currentConfig.scale < 0.1)
							state.currentConfig.scale = 0.1;
						state.renderer.updateGlobalTransform();
					} else {
						state.currentConfig.offsetY += state.isCtrlPressed ? 10 : 1;
						state.renderer.updateGlobalTransform();
	...
	function handleGUIAction(action:String) {
		switch (action) {
			// Edit mode
			case "mode_clippos":
				state.clipEditor.setEditMode(0);
			case "mode_clipsize":
				state.clipEditor.setEditMode(1);
			case "mode_offset":
				state.clipEditor.setEditMode(2);
			case "mode_clipid":
				state.clipEditor.setEditMode(3);
			case "mode_global":
				state.clipEditor.setEditMode(4);
			// Sustain
			case "sustain_toggle":
				Main.current.playScrollSound();
				state.showSustainPreview = !state.showSustainPreview;
			...
			// Gap
			case "gap_m10":
				state.maniaManager.adjustGap(-10);
			case "gap_m1":
				state.maniaManager.adjustGap(-1);
			case "gap_p1":
				state.maniaManager.adjustGap(1);
			case "gap_p10":
				state.maniaManager.adjustGap(10);
	...
	function handleMouseDown(mouseX:Float, mouseY:Float, button:MouseButton) {
		if (!state.showEditor || button != MouseButton.LEFT)
			return;
		...
		if (state.showSustainPreview && !state.spriteSheetMode) {
			...
			for (si in 0...state.sustainSprites.length) {
				...
				if (mouseX >= s.x - hitMargin && mouseX <= s.x + hitMargin && mouseY >= topY - hitMargin && mouseY <= topY + hitMargin) {
...
class NoteskinEditor {
	...
	function setImportButtonLabelsAlpha(alpha:Float) {
		if (saveButtonText != null)
			saveButtonText.alpha = alpha;
		if (importButtonText != null)
			importButtonText.alpha = alpha;
		if (importButton18KText != null)
			importButton18KText.alpha = alpha;
		if (switchNoteskinButtonText != null)
			switchNoteskinButtonText.alpha = alpha;
	}
		...
		if (showEditor) {
			...
			for (s in guiSprites) {
				...
				if (guiSpriteBuf != null)
					guiSpriteBuf.updateElement(s);
			}
			if (guiSpriteBuf != null)
				guiSpriteBuf.update();
			if (guiStateText != null)
				guiStateText.alpha = 1;
			if (guiBackground != null)
				guiBackground.c.aF = 0.7;
			if (guiLeftBackground != null)
				guiLeftBackground.c.aF = 0.7;
			...
			if (guiBackground != null)
				guiBackground.c.aF = 0;
			if (guiLeftBackground != null)
				guiLeftBackground.c.aF = 0;
			if (saveButtonBox != null)
				saveButtonBox.c.aF = 0;
			if (importButtonBox != null)
				importButtonBox.c.aF = 0;
			if (importButton18KBox != null)
				importButton18KBox.c.aF = 0;
			if (switchNoteskinButtonBox != null)
				switchNoteskinButtonBox.c.aF = 0;
			if (gridBuf != null)
				gridBuf.update();
	...
	**/
	public static function preInit(display:CustomDisplay, view:CustomDisplay) {
		...
		if (NoteskinEditor.saveButtonText != null)
			NoteskinEditor.saveButtonText.removeProgram();
		if (NoteskinEditor.importButtonText != null)
			NoteskinEditor.importButtonText.removeProgram();
		if (NoteskinEditor.importButton18KText != null)
			NoteskinEditor.importButton18KText.removeProgram();
		if (NoteskinEditor.switchNoteskinButtonText != null)
			NoteskinEditor.switchNoteskinButtonText.removeProgram();
		if (NoteskinEditor.guiStateText != null)
			NoteskinEditor.guiStateText.removeProgram();
		if (NoteskinEditor.instructionsText != null)
	...
	public function dispose() {
		...
		if (guiSpriteProg != null) {
			if (view != null && guiSpriteProg.isIn(view))
		...
		if (strumline != null) {
			for (i in 0...strumline.length) {
				if (noteBuf != null)
					noteBuf.removeElement(strumline.receptors[i].note);
			}
		}
		if (sustainSprites != null) {
			for (s in sustainSprites) {
				if (s != null && sustainBuf != null)
					sustainBuf.removeElement(s);
			}
		}
		if (gridSprites != null) {
			for (sprite in gridSprites) {
				if (sprite != null && receptorGridBuf != null)
					receptorGridBuf.removeElement(sprite);
			}
		}
