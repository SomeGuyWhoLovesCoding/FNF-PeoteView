...
class TexUtils {
	...
	public static function createEmptyTexture(gl:PeoteGL, width:Int, height:Int, format:TextureFormat, smoothExpand:Bool = false, smoothShrink:Bool = false,
		...
		if (format.isCompressed) {
			var blockSize:Int = 16; // 4x4 block = 16 bytes
			var blocksX = Math.ceil(width / 4);
			var blocksY = Math.ceil(height / 4);
			var dataSize = Std.int(blocksX * blocksY) * blockSize;

			// FIX: Pass a real UInt8Array instead of null to prevent Lime C++ segfaults
			var dummyData = new lime.utils.UInt8Array(dataSize);

			#if js
			gl.compressedTexImage2D(gl.TEXTURE_2D, 0, format.compressedInternalFormat(gl), width, height, 0, dummyData);
			#else
			// Pass the dummy data natively as well
			gl.compressedTexImage2D(gl.TEXTURE_2D, 0, format.compressedInternalFormat(gl), width, height, 0, dataSize, dummyData);
			#end
		...
		// trace("GL ERROR: " + GLTool.getLastGlError(gl));
	...
	public static function dataToTexture(gl:PeoteGL, x:Int, y:Int, format:TextureFormat, textureData:TextureData, genMipmap:Bool, ?glTexture:GLTexture) {
		...
		if (format.isCompressed) {
			var dataSize:Int = textureData.bytes.length;
			var blocksize = 4; // ASTC 4x4

			var uploadWidth = Math.ceil(textureData.width / blocksize) * blocksize;
			var uploadHeight = Math.ceil(textureData.height / blocksize) * blocksize;

			#if js
			// FIX: explicitly convert to UInt8Array
			gl.compressedTexSubImage2D(gl.TEXTURE_2D, 0, x, y, uploadWidth, uploadHeight, format.compressedInternalFormat(gl), textureData.toUInt8Array());
			#else
			// FIX: explicitly pass textureData.bytes to avoid abstract unwrapping segfaults
			gl.compressedTexSubImage2D(gl.TEXTURE_2D, 0, x, y, uploadWidth, uploadHeight, format.compressedInternalFormat(gl), dataSize, textureData.bytes);
			#end
		} else if (format.isFloat) {
			...
			gl.texSubImage2D(gl.TEXTURE_2D, 0, x, y, textureData.width, textureData.height, format.formatInteger(gl), gl.UNSIGNED_BYTE, textureData);
		}

		if (GLTool.getLastGlError(gl) == gl.OUT_OF_MEMORY)
			throw("OUT OF GPU MEMORY while texture creation");

		if (genMipmap && !format.isCompressed)
			createMipmap(gl);
	...
	public static inline function setMinMagFilter(gl:PeoteGL, ?smoothExpand:Null<Bool>, ?smoothShrink:Null<Bool>, ?smoothMipmap:Null<Bool>,
			?glTexture:GLTexture) {
		if (glTexture != null)
			gl.bindTexture(gl.TEXTURE_2D, glTexture);
	...
	public static function createPickingTexture(gl:PeoteGL, isRGBA32I:Bool = false):GLTexture {
		...
		if (isRGBA32I)
			gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32I, 1, 1, 0, gl.RGBA_INTEGER, gl.INT, 0);
		else
			gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, 0);
	...
	public static function optimalTextureSize(slots:Int, slotWidth:Int, slotHeight:Int, maxTextureSize:Int, powerOfTwo = true, errorIfNotFit = true,
			debug = true):{
		width:Int,
		height:Int,
		slotsX:Int,
		...
		if (powerOfTwo)
			return optimalTextureSizePowerTwo(slots, slotWidth, slotHeight, maxTextureSize, errorIfNotFit, debug);

		if (slots < 1)
			throw('Error: the slots have to be greater then 1');
		else if (slots == 1) {
		...
		if (aspect == null || aspect != 1.0) {
			...
			if (aspect == null && _aspect == null) {
				if (errorIfNotFit)
					throw('Error: max texture-size ($maxTextureSize) is to small for $slots images ($slotWidth x $slotHeight)');
				w = wMax;
				h = hMax;
	...
	static inline function optimalTextureSizePowerTwo(slots:Int, slotWidth:Int, slotHeight:Int, maxTextureSize:Int, errorIfNotFit = true, debug = true):{
		width:Int,
		height:Int,
		slotsX:Int,
		slotsY:Int
		...
		while ((1 << n) >= slotWidth) {
			...
			while ((1 << m) >= slotHeight) { // trace('  $n,$m - ${1<<n} w ${1<<m}');
		...
		if (found) {
			...
			if (errorIfNotFit)
				throw('Error: max texture-size ($maxTextureSize) is to small for $slots images (a $slotWidth x $slotHeight) and powerOfTwo rounding up');
			if (slotWidth > maxTextureSize || slotHeight > maxTextureSize)
				throw('Error: max texture-size ($maxTextureSize) is to small for image ($slotWidth x $slotHeight) and powerOfTwo rounding up');
			w = h = maxTextureSize;
		}
