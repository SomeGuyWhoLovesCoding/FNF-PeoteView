...
class Texture {
	...
	**/
	...
	inline function set_smoothMipmap(b:Bool):Bool {
		if (gl != null)
			TexUtils.setMinMagFilter(gl, null, smoothShrink, (mipmap) ? smoothMipmap : null, glTexture);
	...
	**/
	...
	inline function set_smoothExpand(b:Bool):Bool {
		if (gl != null)
			TexUtils.setMinMagFilter(gl, smoothExpand, null, null, glTexture);
	...
	**/
	...
	/**
		The texture color will be cleared before a `Display` is rendering into.
	**/
	public var clearOnRenderInto = true;

	/**
		Clears the depth-buffer by `clearDepthIndex` value before a `Display` is rendering into this texture.
	**/
	public var clearDepthOnRenderInto = true;

	/**
		Index for initializing the depth buffer when `clearDepthOnRenderInto` is enabled.
	**/
	public var clearDepthIndex(get, set):Int;

	inline function get_clearDepthIndex():Int
		return Math.round((0.5 - clearDepthValue) * 0x1FFFFF * 2.0);
	...
	**/
	public function new(slotWidth:Int, slotHeight:Int, ?slots:Null<Int>, ?textureConfig:TextureConfig) {
		...
		if (textureConfig.format.isCompressed) {
			var blocksize = 4; // ASTC_44
			slotWidth = Math.ceil(slotWidth / blocksize) * blocksize;
			slotHeight = Math.ceil(slotHeight / blocksize) * blocksize;
		}
		...
		if (slots == null) {
			...
			if (width > textureConfig.maxTextureSize || height > textureConfig.maxTextureSize)
				throw('Error: max texture-size (${textureConfig.maxTextureSize}) is to small for ${this.slots} images ($width x $height)${(textureConfig.powerOfTwo) ? "<-powerOfTwo!" : ""}');
		} else {
	...
	**/
	public inline function usedByProgram(program:Program):Bool
		return (programs.indexOf(program) >= 0);

	private inline function addToProgram(program:Program) {
		#if peoteview_debug_texture
		trace("Add Program to Texture");
		#end
		if (usedByProgram(program))
			throw("Error, texture is already used by program");
	...
	private inline function removeFromProgram(program:Program) {
		...
		if (!programs.remove(program))
			throw("Error, this texture is not used by program anymore");
	}
	...
	**/
	public inline function usedByDisplay(display:Display):Bool
		return (displays.indexOf(display) >= 0);

	private inline function addToDisplay(display:Display) {
		#if peoteview_debug_texture
		trace("Add Display to Texture");
		#end
		if (usedByDisplay(display))
			throw("Error, texture is already used by display");
	...
	private inline function removeFromDisplay(display:Display) {
		...
		if (!displays.remove(display))
			throw("Error, this texture (Framebuffer) is not used by display anymore");
		deleteFramebuffer();
	}
	...
	private inline function createFramebuffer() {
		if (displays.length > 0 && framebuffer == null) {
			...
			if (PeoteGL.Version.isES3 && format.isFloat) {
				if (gl.getExtension("EXT_color_buffer_float") != null) {} else if (gl.getExtension("OES_texture_float") != null) {}
			}
		...
		if (newGl != null && newGl != gl) // only if different GL - Context
		{
			...
			for (p in programs)
				if (p.gl != null && p.gl != newGl)
					throw("Error, texture can not used inside different gl-contexts");
			for (d in displays)
	...
	**/
	public function readPixelsUInt8(x:Int, y:Int, w:Int, h:Int, ?data:UInt8Array):UInt8Array {
		if (programs == null)
			throw("Error, texture is disposed.");
		if (gl == null)
			throw("Error, texture have no data yet.");
		if (format.isFloat)
			throw('Error, for float textureformat you have to use "readPixelsFloat32()".');
		if (data == null)
	...
	**/
	public function readPixelsFloat32(x:Int, y:Int, w:Int, h:Int, ?data:Float32Array):Float32Array {
		...
		if (gl == null)
			throw("Error, texture have no data yet.");
		if (!format.isFloat)
			throw('Error, for integer textureformat you have to use "readPixelsUInt8()".');
		if (data == null)
	...
	**/
	public function setData(textureData:TextureData, slot:Int = 0) {
		...
		if (format != textureData.format)
			trace("Warning: Textureformat of Texture and TextureData don't match");
		#end
	...
	**/
	public function setSmooth(smoothExpand:Bool, smoothShrink:Bool, smoothMipmap:Null<Bool> = null) {
		...
		if (gl != null)
			TexUtils.setMinMagFilter(gl, smoothExpand, smoothShrink, (mipmap) ? this.smoothMipmap : null, glTexture);
	}
	...
	**/
	public function dispose() {
		if (programs == null)
			throw("Error, texture is already disposed.");
		#if peoteview_debug_texture
		trace("Texture is disposed");
		#end
		// remove from all Programs
		for (p in programs)
			p.removeTexture(this);
