...
class Texture
{
	...
	**/
	public var textureDataSlots(default, null):Array<TextureData>;
	...
	**/
	public function new(slotWidth:Int, slotHeight:Int, ?slots:Null<Int>, ?textureConfig:TextureConfig, createWhenDataAdded:Bool = false)
	{
        BAD2 = !createWhenDataAdded;
		textureDataSlots = [for (i in 0...100) null];
	...
	private inline function setNewGLContext(newGl:PeoteGL) {
		if (newGl != null && newGl != gl) // only if different GL - Context	
		{
            ...
            if (BAD2) {
                ...
                for (slot in 0...textureDataSlots.length) {
                    var textureData = textureDataSlots[slot];
...
*/
	...
	**/
	public function setData(textureData:TextureData, slot:Int = 0) {
		...
		textureDataSlots[slot] = textureData;

		if (gl != null) {
			// TODO: optimize here to also setData while creation if there is only 1 slot and textureData already set
			if (glTexture == null) {
                if (!BAD2)
                    glTexture = TexUtils.createEmptyTexture(gl, slotWidth * (slot % slotsX), slotHeight * Std.int(slot / slotsX), format, smoothExpand, smoothShrink, mipmap, smoothMipmap, textureData);
                else
                    createTexture();
            } else
	...
	**/
	public function clearSlot(slot:Int) {
		...
		#end
		var textureData = textureDataSlots[slot];
		textureDataSlots.remove(textureDataSlots[slot]);
