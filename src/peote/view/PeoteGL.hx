...
abstract PeoteGL(LimeGLRenderContext) from LimeGLRenderContext to LimeGLRenderContext {
	...
	@:from private static function fromRenderContext(context:lime.graphics.RenderContext):PeoteGL {
		...
		if (context.webgl2 == null)
			js.Browser.alert("Sorry, only works on Webbrowsers that supports WEBGL2 (OpenGL-ES3).");
		trace("Force WEBGL2.");
		return cast context.webgl2;
		#elseif peoteview_es2
		if (context.webgl == null)
			js.Browser.alert("Sorry, only works on Webbrowsers that supports WEBGL1 (OpenGL-ES2).");
		trace("Force WEBGL1.");
		...
		if (context.gles3 == null)
			throw("Sorry, only works with OpenGL-ES3.");
		trace("Force OpenGL-ES3.");
		return cast context.gles3;
		#elseif peoteview_es2
		if (context.gles2 == null)
			throw("Sorry, only works with OpenGL-ES2.");
		...
		if (context.gles3 != null) {
			...
			throw("Error: missing OpenGL context");
		#end
		#end
	}
}
...
class Precision {
	...
	static public inline function highest(t:PrecType, p:Null<String>):Null<String> {
		...
		if (t.high > 0 && p == "highp")
			return "highp";
		else if (t.medium > 0 && (p == "highp" || p == "mediump"))
			return "mediump";
		else if (t.low > 0)
			return "lowp";
		#if html5
	...
	static public inline function availVertexFloat(precision:Null<String>):Null<String>
		return highest(VertexFloat, precision);

	static public inline function availVertexInt(precision:Null<String>):Null<String>
		return highest(VertexInt, precision);

	static public inline function availVertexSampler(precision:Null<String>):Null<String>
		return highest(VertexSampler, precision);

	static public inline function availFragmentFloat(precision:Null<String>):Null<String>
		return highest(FragmentFloat, precision);

	static public inline function availFragmentInt(precision:Null<String>):Null<String>
		return highest(FragmentInt, precision);

	static public inline function availFragmentSampler(precision:Null<String>):Null<String>
		return highest(FragmentSampler, precision);
}
