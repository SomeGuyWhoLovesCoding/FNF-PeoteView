...
class TextureDataImpl {
	...
	public function new(width:Int, height:Int, format:TextureFormat = TextureFormat.RGBA, color:Color = 0, ?bytes:Bytes) {
		...
		if (bytes == null) {
			if (format.isCompressed) {
				...
				this.bytes = Bytes.alloc(Std.int(blocksX * blocksY) * 16);
			} else {
	...
	**/
	public function clear(color:Color = 0) {
		...
		if (format.isFloat)
			clearFloat(color.redF, color.greenF, color.blueF, color.alphaF);
		else {
			var pos:Int = 0;
			switch (format) {
				case RGBA:
	...
	**/
	public function clearFloat(red:Float = 0.0, green:Float = 0.0, blue:Float = 0.0, alpha:Float = 0.0) {
		if (format.isCompressed)
			throw("error, use clear() for compressed textureformats");
		if (!format.isFloat)
			throw("error, use clear() for INTEGER textureformats");
		var pos:Int = 0;
		switch (format) {
			case FLOAT_RGBA:
				for (i in 0...width * height) {
					...
					pos += 4;
					bytes.setFloat(pos, green);
					pos += 4;
					bytes.setFloat(pos, blue);
					pos += 4;
					bytes.setFloat(pos, alpha);
					pos += 4;
				}
			case FLOAT_RGB:
				for (i in 0...width * height) {
	...
	**/
	public function toRGBA():TextureData {
		if (format.isCompressed)
			throw("Error: CPU-side conversion from compressed texture is not supported without a decoder.");
		...
		switch (format) {
			...
			case FLOAT_RGBA:
				while (d < t.bytes.length) {
					t.bytes.set(d++, Std.int(bytes.getFloat(s) * 255));
					s += 4;
					t.bytes.set(d++, Std.int(bytes.getFloat(s) * 255));
					s += 4;
					t.bytes.set(d++, Std.int(bytes.getFloat(s) * 255));
		...
		switch (format) {
			...
			case FLOAT_RGBA:
				while (d < t.bytes.length) {
					...
					t.bytes.set(d++, Std.int(bytes.getFloat(s += 4) * 255));
					s += 4;
				}
			case FLOAT_RGB:
				while (d < t.bytes.length) {
			...
			case FLOAT_RG:
				while (d < t.bytes.length) {
					t.bytes.set(d++, Std.int((bytes.getFloat(s) + bytes.getFloat(s += 4)) * 127));
					s += 4;
					t.bytes.set(d++, 0xff);
				}
			case FLOAT_R:
	...
	**/
	public function toAlpha():TextureData {
		...
		switch (format) {
			...
			case FLOAT_RGBA:
				while (d < t.bytes.length) {
					t.bytes.set(d++, Std.int(bytes.getFloat(s += 12) * 255));
					s += 4;
				}
			case FLOAT_RGB:
				while (d < t.bytes.length) {
	...
	**/
	public function toFloatRGBA():TextureData {
		...
		switch (format) {
			...
			case FLOAT_RGB:
				while (d < t.bytes.length) {
					t.bytes.setFloat(d, bytes.getFloat(s));
					d += 4;
					s += 4;
					t.bytes.setFloat(d, bytes.getFloat(s));
					d += 4;
			...
			case RGBA:
				while (d < t.bytes.length) {
					t.bytes.setFloat(d, bytes.get(s++) / 255);
					d += 4;
					t.bytes.setFloat(d, bytes.get(s++) / 255);
					d += 4;
					t.bytes.setFloat(d, bytes.get(s++) / 255);
			...
			case ALPHA:
				while (d < t.bytes.length) {
					t.bytes.setFloat(d += 12, bytes.get(s++) / 255);
					d += 4;
				}
			default:
		}
	...
	**/
	public function setColor(x:Int, y:Int, color:Color) {
		if (format.isCompressed)
			throw("Error: Cannot set pixels directly on compressed data.");
		if (format.isFloat)
			throw("error, use setFloat() for FLOAT textureformats");
		var pos = (y * width + x) * format.bytesPerPixelInt;
		switch (format) {
			case RGBA:
				_setRGBABytes(pos, color);
			case LUMINANCE:
				bytes.set(pos, color.luminance);
			case ALPHA:
				bytes.set(pos, color.alpha);
	...
	/**
		...
		@param color color value
	**/
	public inline function setColor_RGBA(x:Int, y:Int, color:Color)
		_setRGBABytes((y * width + x) << 2, color);
	...
	**/
	public inline function set_RGBA(x:Int, y:Int, red:Int, green:Int, blue:Int, alpha:Int) {
		var pos = (y * width + x) * 3;
		bytes.set(pos, red);
		bytes.set(pos + 1, green);
		bytes.set(pos + 2, blue);
		bytes.set(pos + 3, alpha);
	}
	...
	**/
	...
	/**
		...
		@param red value from 0 to 255
	**/
	public inline function set_R(x:Int, y:Int, red:Int)
	...
	/**
		...
		@param alpha value from 0 (full transparent) to 255 (opaque)
	**/
	public inline function set_LUMINANCE_ALPHA(x:Int, y:Int, luminance:Int, alpha:Int)
		set_RG(x, y, luminance, alpha);
	...
	/**
		...
		@param luminance value from 0 to 255
	**/
	public inline function set_LUMINANCE(x:Int, y:Int, luminance:Int)
		set_R(x, y, luminance);
	...
	/**
		...
		@param alpha value from 0 (full transparent) to 255 (opaque)
	**/
	public inline function set_ALPHA(x:Int, y:Int, alpha:Int)
		set_R(x, y, alpha);
	...
	**/
	public inline function setRed(x:Int, y:Int, red:Int) {
		if (format.isInRGBA())
			...
			throw("Error: Textureformat have no red channel");
	}
	...
	**/
	public inline function setGreen(x:Int, y:Int, green:Int) {
		if (format.isGreaterR() && format.isInRGBA())
			bytes.set((y * width + x) * format.bytesPerPixelInt + 1, green);
		else
			throw("Error: Textureformat have no green channel");
	}
	...
	**/
	public inline function setBlue(x:Int, y:Int, blue:Int) {
		if (format.isGreaterRG() && format.isInRGBA())
			bytes.set((y * width + x) * format.bytesPerPixelInt + 2, blue);
		else
			throw("Error: Textureformat have no blue channel");
	}
	...
	**/
	public inline function setAlpha(x:Int, y:Int, alpha:Int) {
		switch (format) {
			case RGBA:
				bytes.set((y * width + x) * 4 + 3, alpha);
			case LUMINANCE_ALPHA:
				bytes.set((y * width + x) * 2 + 1, alpha);
			case ALPHA:
				bytes.set((y * width + x), alpha);
			default:
				throw("Error: Textureformat have no alpha channel");
		}
	}
	...
	public inline function setLuminance(x:Int, y:Int, luminance:Int) {
		...
		switch (format) {
			...
			case LUMINANCE_ALPHA:
				bytes.set(pos, luminance);
			case ALPHA:
				throw("Error: Textureformat have no red, green or blue channel to set luminance");
			default:
				bytes.set(pos, luminance);
				if (format.isGreaterR()) {
	...
	**/
	public function getColor(x:Int, y:Int):Color {
		if (format.isCompressed)
			throw("Error: Cannot get pixels directly from compressed data.");
		if (format.isFloat)
			throw("error, use getFloat...() for FLOAT textureformats");
		var pos = (y * width + x) * format.bytesPerPixelInt;
	...
	/**
		...
		@param y vertical position
	**/
	public inline function getColor_RGBA(x:Int, y:Int):Color
		return _getRGBABytes((y * width + x) << 2);
	...
	**/
	...
	/**
		...
		@param y vertical position
	**/
	public inline function getColor_R(x:Int, y:Int):Color
		return Color.Red(get_R(x, y));
	...
	/**
		...
		@param y vertical position
	**/
	public inline function getColor_LUMINANCE(x:Int, y:Int):Color
		return Color.Luminance(get_R(x, y));
	...
	/**
		...
		@param y vertical position
	**/
	public inline function getColor_ALPHA(x:Int, y:Int):Color
		return Color.Alpha(get_R(x, y));
	...
	/**
		...
		@param y vertical position
	**/
	public inline function get_R(x:Int, y:Int):Int
	...
	/**
		...
		@param y vertical position
	**/
	public inline function get_LUMINANCE(x:Int, y:Int):Int
		return get_R(x, y);
	...
	/**
		...
		@param y vertical position
	**/
	public inline function get_ALPHA(x:Int, y:Int):Int
		return get_R(x, y);
	...
	**/
	public inline function setFloat(x:Int, y:Int, red:Float, green:Float, blue:Float, alpha:Float = 0.0) {
		if (format.isCompressed)
			throw("Error: Cannot set pixels directly on compressed data.");
		if (!format.isFloat)
			throw("error, use setPixel() for INTEGER textureformats");
	...
	**/
	public inline function setFloatGreen(x:Int, y:Int, green:Float) {
		if (format.isGreaterFloatR())
			bytes.setFloat(((y * width + x) * format.bytesPerPixelFloat) + 4, green);
		else
			throw("Error: Textureformat have no green channel");
	}
	...
	**/
	public inline function setFloatBlue(x:Int, y:Int, blue:Float) {
		if (format.isGreaterFloatRG())
			bytes.setFloat(((y * width + x) * format.bytesPerPixelFloat) + 8, blue);
		else
			throw("Error: Textureformat have no blue channel");
	}
	...
	**/
	public inline function setFloatAlpha(x:Int, y:Int, alpha:Float) {
		if (format.isGreaterFloatRGB())
			bytes.setFloat(((y * width + x) * format.bytesPerPixelFloat) + 12, alpha);
		else
			throw("Error: Textureformat have no alpha channel");
	}
...
abstract TextureData(TextureDataImpl) from TextureDataImpl to TextureDataImpl {
	...
	**/
	static public inline function RGBAfrom(textureData:TextureData):TextureData
		return textureData.toRGBA();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function RGBfrom(textureData:TextureData):TextureData
		return textureData.toRGB();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function RGfrom(textureData:TextureData):TextureData
		return textureData.toRG();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function Rfrom(textureData:TextureData):TextureData
		return textureData.toR();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function LuminanceAlphaFrom(textureData:TextureData):TextureData
		return textureData.toLuminanceAlpha();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function LuminanceFrom(textureData:TextureData):TextureData
		return textureData.toLuminance();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function AlphaFrom(textureData:TextureData):TextureData
		return textureData.toAlpha();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function FloatRGBAfrom(textureData:TextureData):TextureData
		return textureData.toFloatRGBA();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function FloatRGBfrom(textureData:TextureData):TextureData
		return textureData.toFloatRGB();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function FloatRGfrom(textureData:TextureData):TextureData
		return textureData.toFloatRG();
	...
	/**
		...
		@param textureData source data to convert from
	**/
	static public inline function FloatRfrom(textureData:TextureData):TextureData
		return textureData.toFloatR();
	...
	**/
	static public function fromFormatPNG(bytes:Bytes):TextureData {
		...
		switch (header.color) {
			...
			case ColIndexed:
				throw("Error by decoding PNG for TextureData: indexed colors not supperted");
		}
