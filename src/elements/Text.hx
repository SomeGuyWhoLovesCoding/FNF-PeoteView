	...
	function set_text(raw:String) {
		...
		for (lineIdx in 0..._lines.length) {
			...
			for (i in 0...line.length) {
				...
				var span = (spanIdx < colorSpans.length && rawCharIdx >= colorSpans[spanIdx].start) ? colorSpans[spanIdx] : null;
		...
		while (i < raw.length) {
			...
			if (matched != -1) {
				...
				if (openMarkerIdx == -1) {
					...
					spans.push(new ColorSpan(openCharPos, charPos, mp.color, mp.outlineColor, mp.outlineSize));
					openMarkerIdx = -1;
					openCharPos = -1;
					i += mp.marker.length;
				} else {
