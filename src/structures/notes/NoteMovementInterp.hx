...
class NoteMovementInterp {
	...
	function preprocess(luaCode:String):String {
		if (luaCode == null || StringTools.trim(luaCode) == "")
			return "";
		var code = luaCode;
	...
	function tokenize(codeStr:String):Array<String> {
		...
		while (i < len) {
			...
			if (c == 32 || c == 9 || c == 10 || c == 13) { // space, tab, newline, cr
				if (i > start)
					tokens.push(codeStr.substring(start, i));
				start = i + 1;
	...
	function getVarIndex(name:String):Int {
		...
		switch (lower) {
			...
			case "scale":
				idx = 2;
			case "sustainrot" | "sustain_rotation":
				idx = 3;
			case "scrollmultiplier" | "scrol_multiplier":
				idx = 4;
			case "diff":
				idx = 5;
			case "scrollspeed" | "scroll_speed":
				idx = 6;
			case "receptorx" | "receptor_x":
				idx = 7;
			case "receptory" | "receptor_y":
				idx = 8;
			case "index":
				idx = 9;
			case "type":
	...
	inline function isFunction(lower:String):Bool {
		return lower == "sin" || lower == "cos" || lower == "min" || lower == "max" || lower == "abs" || lower == "math.sin" || lower == "math.cos"
			|| lower == "math.min" || lower == "math.max" || lower == "math.abs" || lower == "_ab" || lower == "_ab_neq" || lower == "_ab_eq";
	}
	...
	function isPrevTokenOperator(tokens:Array<String>, pos:Int):Bool {
		...
		if (len == 1) {
			...
			// ( = , ; + - * / %
			if (c == 40 || c == 61 || c == 44 || c == 59 || c == 43 || c == 45 || c == 42 || c == 47 || c == 37)
				return true;
		}
	...
	inline function precedence(op:String):Int {
		return switch (op) {
			...
			case "+", "-": 3;
			case "*", "/", "%": 4;
			case "not", "sin", "cos", "min", "max", "abs", "math.sin", "math.cos", "math.min", "math.max", "math.abs", "_ab", "_ab_neq", "_ab_eq": 5;
			default: 0;
		}
	}
	...
	// ------------------------------------------------------------------
	public function run(diff:Float, scrollSpeed:Float, receptorX:Float, receptorY:Float, index:Float, type:Float,
		...
		while (blockPtr < blocksLen) {
			...
			switch (op) {
				case 0x10:
					stack[stackPtr++] = constants[args[argPtr++]];
				case 0x11:
					stack[stackPtr++] = locals[args[argPtr++]];
				case 0x20:
					locals[args[argPtr++]] = stack[--stackPtr];
				case 0x30:
					return null;
				case 0x31:
				...
				case 0x06:
					stack[stackPtr - 1] = Math.cos(stack[stackPtr - 1]);
				case 0x07:
					var b = stack[--stackPtr];
					if (b < stack[stackPtr - 1])
						stack[stackPtr - 1] = b;
				case 0x08:
					var b = stack[--stackPtr];
					if (b > stack[stackPtr - 1])
				...
				case 0x0A:
					stack[stackPtr - 1] = stack[stackPtr - 1] == 0 ? 1.0 : 0.0;
				case 0x0B:
					var b = stack[--stackPtr];
					var a = stack[stackPtr - 1];
					stack[stackPtr - 1] = a != 0 ? b : 0.0;
				case 0x0C:
					var b = stack[--stackPtr];
					var a = stack[stackPtr - 1];
					stack[stackPtr - 1] = a != 0 ? a : b;
				case 0x33:
					{} // NOP
			}
