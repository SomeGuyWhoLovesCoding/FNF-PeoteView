...
class EventSystem {
	...
	function update(deltaTime:Float, songTime:Float) {
		...
		for (timer in eventTimers) {
			if (songTime < timer.startTime || songTime > timer.endTime)
				continue;
	...
	function triggerEvent(ev:EventObject) {
		...
		if (parent.funkinviewlua != null)
			parent.funkinviewlua.callFunction('preTriggerEvent', ev.evName, ev.value1, ev.value2);
		#end
		...
		switch (ev.evName) {
			...
			case "Screen Shake":
				...
				eventTimers.push(new EventTimer(ev.evTime, ev.evTime + (time * 1000.0), ev, (ev) -> {
	...
	function processEventTimer(eventTimer:EventTimer) {
		...
		switch (ev.evName) {
			case "Screen Shake":
				...
				if (Math.isNaN(dispSplit))
					dispSplit = 0;
				if (Math.isNaN(viewSplit))
					viewSplit = 0;
