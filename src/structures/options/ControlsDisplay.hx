...
class ControlsDisplay implements IAlphabetScrollHost {
	...
	inline function get_curManiaNum() return curManiaNum;
	...
	function onKeyDown(keyCode:KeyCode, keyModifier:KeyModifier) {
		...
		if (bindingMania) {
			...
			if ((keyModifier == KeyModifier.LEFT_CTRL || keyModifier == KeyModifier.RIGHT_CTRL)) {
				switch (keyCode) {
					case KeyCode.LEFT: curManiaNum--; Main.current.playScrollSound();
					case KeyCode.RIGHT: curManiaNum++; Main.current.playScrollSound();
					default:
				}
				return;
			}
			if ((keyModifier == KeyModifier.LEFT_ALT || keyModifier == KeyModifier.RIGHT_ALT)) {
				...
				switch (keyCode) {
					case KeyCode.UP: maniaBindNum = Std.int(Math.max(0, maniaBindNum - 1)); Main.current.playScrollSound();
					case KeyCode.DOWN: maniaBindNum = Std.int(Math.min(keybindsArr.length - 1, maniaBindNum + 1)); Main.current.playScrollSound();
					default:
				}
				return;
			}
	...
	inline function isInvalidKey(keyCode:KeyCode) {
		var result = false;
		result =
			(keyCode >= 0x4000003A && keyCode < 0x40000045) ||
