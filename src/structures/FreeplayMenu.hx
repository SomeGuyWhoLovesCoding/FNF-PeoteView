...
import data.chart.Header;
import elements.actor.sparrow.Actor;
import elements.sprites.HealthBarSprite;
import data.gameplay.ChapterData.ChapterSong;
...
class FreeplayMenu {
	...
	var curSelected(default, null):Int = 0;
	var sectionLen(default, null):Int = 0;
	...
	function new() {
		...
		sectionLen = Math.floor(Math.max(songsAvailable.length, 8));
	...
	function update(deltaTime:Float) {
		...
		for (i in 0...sectionLen) {
			...
			var x:Float = 45;
			var y:Float = ((-curSelectedLerp * 156) + (156 * k) + 320);
			...
			var icon = songIconGroup[kClamped];
			icon.changeID(Tools.fromIconGridXMLCharacter(song.icon)[0]);
			icon.c.aF = (i == l ? 1.0 : 0.5) * alphaLerp;
			icon.x = (xLerp + (20 * k)) + 12;
			icon.y = y + 15;
			songIconsBuf.updateElement(icon);
		}
